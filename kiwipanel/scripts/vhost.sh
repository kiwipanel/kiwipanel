...
# Get custom document root (optional)
CUSTOM_DOCROOT=""
if [[ -n "${2:-}" ]]; then
    CUSTOM_DOCROOT="$2"
else
    read -p "Custom document root (press Enter for default): " CUSTOM_DOCROOT
fi

# Determine paths based on custom docroot
if [[ -n "$CUSTOM_DOCROOT" ]]; then
    # User provided custom path
    DOCROOT="${CUSTOM_DOCROOT%/}"  # Remove trailing slash
    
    # Validate custom path
    if [[ "$DOCROOT" == "/" ]] || [[ "$DOCROOT" == "/root" ]]; then
        print_error "Cannot use root directories as document root"
    fi
    
    # For custom paths, we need a separate location for vhost config and logs
    VHOST_ROOT="$LSWS_DIR/vhosts/$VHOST_NAME"
else
    # Default structure
...
print_info "VHost Root: $VHOST_ROOT"
echo ""
...
# Detect if using custom location outside /usr/local/lsws
CUSTOM_LOCATION=0
if [[ "$DOCROOT" != "$LSWS_DIR"* ]]; then
    CUSTOM_LOCATION=1
    print_info "✓ Using custom document root location"
fi
...
if [[ $CUSTOM_LOCATION -eq 1 ]]; then
    # For custom locations (like /home/*), use current user or www-data
    if [[ "$DOCROOT" == /home/* ]]; then
        # Extract username from /home/username/...
        OWNER=$(echo "$DOCROOT" | cut -d'/' -f3)
        if id "$OWNER" &>/dev/null; then
            chown -R "$OWNER:$OWNER" "$DOCROOT"
            print_info "Set ownership to: $OWNER:$OWNER"
        else
            chown -R lsadm:lsadm "$DOCROOT"
            print_warn "User $OWNER not found, using lsadm:lsadm"
        fi
    else
        chown -R lsadm:lsadm "$DOCROOT"
    fi
    
    # Ensure OpenLiteSpeed can read the directory
    chmod -R 755 "$DOCROOT"
    
    # Logs still go to vhost root under lsws
    chown -R lsadm:lsadm "$VHOST_ROOT/logs"
else
    # Standard location - use lsadm
...
if [[ $CUSTOM_LOCATION -eq 1 ]]; then
    print_warn "Custom Location Notes:"
    echo "  • Ensure the web server can read: $DOCROOT"
    echo "  • Parent directories must be executable (chmod +x)"
    echo "  • Files should be readable (chmod 644)"
    echo "  • Directories should be readable+executable (chmod 755)"
    echo ""
fi
